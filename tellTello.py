...
import os
import time
import getch                   # works for windows only. Linux users, import getch instead (not tested)
import argparse
import subprocess
...
#-----------------------------------------------------------------------------------
def ping(ipadress,t=1, c=1):
    timeout = str(t)
    count = str(c)
    try:
        subprocess.check_output(["ping", "-c", count, "-w", timeout, ipadress])
        return True                      
